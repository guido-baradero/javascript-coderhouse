...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Templates
    const diaTemplate = document.getElementById('diaTemplate');
    const semanaTemplate = document.getElementById('semanaTemplate');
    const mesTemplate = document.getElementById('mesTemplate');
    ...
    function mostrarTareas(filtro) {
        ...
        console.table(tareasFiltradas); // Mostrar las tareas filtradas en la consola

        if (filtro === 'dia') {
            const diaClone = diaTemplate.content.cloneNode(true);
            const tareasDia = diaClone.querySelector('.tareas-dia');
                    ...
                    <div class="accordion-item">
                        ...
                        <div id="collapse${tarea.id}" class="accordion-collapse collapse"
                            ...
                            <div class="accordion-body">
                                ...
                                <p>Tipo: ${tarea.tipo}</p>
                                <p>Hora: ${tarea.hora}</p>
                                <p>Días de repetición: ${tarea.diasRepeticion ? tarea.diasRepeticion.join(', ') : ''}</p>
            ...
            `;
                tareasDia.appendChild(tareaDiv);
            });
            contenedorTareas.appendChild(diaClone);
        } else if (filtro === 'semana') {
            const semanaClone = semanaTemplate.content.cloneNode(true);
            const diasSemana = ['lunes', 'martes', 'miercoles', 'jueves', 'viernes', 'sabado', 'domingo'];

            tareasFiltradas.forEach(tarea => {
                tarea.diasRepeticion.forEach(dia => {
                    const diaDiv = semanaClone.querySelector(`#${dia.toLowerCase()}`);
                    const tareaDiv = document.createElement('div');
                    tareaDiv.classList.add('tarea');
                    tareaDiv.innerHTML = `
                        <div class="accordion" id="accordionExample">
            ...
            contenedorTareas.appendChild(semanaClone);
        } else if (filtro === 'mes') {
            for (let i = 1; i <= 31; i++) {
